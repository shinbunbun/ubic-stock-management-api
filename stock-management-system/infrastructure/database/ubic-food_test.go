...
func TestReplaceItems(t *testing.T) {
	...
	id, err := h.AddMultipleItems(widgets)
	checkError(t, err, nil)

	t.Run("Successful replace with 1 item", func(t *testing.T) {
		err := h.ReplaceItems(id, widgets[:1])
		checkError(t, err, nil)
		ws, err := h.GetByID(id)
		checkError(t, err, nil)
		if len(ws) != 1 {
	...
	t.Run("Successful replace with 3 item", func(t *testing.T) {
		add := UbicFoodWidget{
			ID:       "",
			DataType: "type3",
			Data:     "data3",
		...
		err := h.ReplaceItems(id, append(widgets, add))
		checkError(t, err, nil)
		ws, err := h.GetByID(id)
		checkError(t, err, nil)
		if len(ws) != 3 {
